...
const CrearMateria3 = ({ navigation, route }) => {
    ...
    const [showPicker, setShowPicker] = useState({ show: false, dia: null, campo: null });

    const handleTimeChange = (event, selectedDate) => {
        if (event.type === 'dismissed') {
            setShowPicker({ show: false, dia: null, campo: null });
            return;
        }
        const hora = selectedDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setHorarios((prev) => ({
            ...prev,
            [showPicker.dia]: {
                ...prev[showPicker.dia],
                [showPicker.campo]: hora,
            },
        }));
        setShowPicker({ show: false, dia: null, campo: null });
    };
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
            <View style={styles.overlay}>
                <View style={styles.modalContainer}>
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate("CrearMateria2")} style={styles.closeButton}>
                        <Image source={left} style={styles.closeText} />
                    </TouchableOpacity>
                    <Text style={styles.title}>Materia</Text>

                    <Text style={styles.correoText}>¿Cuáles son los horarios?</Text>
                    {/* Contenido desplazable */}
                    <ScrollView style={{ flex: 1, width: '100%', marginBottom: -80}}>
                                ...
                                {openDia === dia && (
                                    <View style={styles.pickerContainer}>
                                        <TouchableOpacity
                                            ...
                                            onPress={() => setShowPicker({ show: true, dia, campo: 'inicio' })}
                                        >
                                            <Text style={styles.horaTexto}>Empieza: {horarios[dia]?.inicio || '--:--'}</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            style={styles.horaButton}
                                            onPress={() => setShowPicker({ show: true, dia, campo: 'fin' })}
                                        >
                                            <Text style={styles.horaTexto}>Termina: {horarios[dia]?.fin || '--:--'}</Text>
                                        </TouchableOpacity>
                                    </View>
                                )}
                            </View>
                    ...
                    {/* Parte fija abajo: progreso + siguiente */}
                    <View style={styles.bottomThing}>
                        ...
                        <TouchableOpacity
                            onPress={() => navigation.navigate("CrearMateria4", {
