...
const HomeMaestro = () => {
    ...
    const cargarMaterias = async () => {
        try {
            ...
            const materiasDelMaestro = snapshot.docs
                .filter(doc => doc.data().profesorId === usuario.uid)
                .map(doc => ({ id: doc.id, ...doc.data() }));
            setMaterias(materiasDelMaestro);
        } catch (error) {
            console.error("Error cargando materias del maestro:", error);
        } finally {
    ...
    const cargarEventos = async () => {
        try {
            ...
            console.error("Error cargando eventos del maestro:", error);
        } finally {
            ...
            {!cargandoMaterias && !cargandoEventos && materias.length === 0 && eventos.length === 0 ? (
                <View style={styles.emptyContainer}>
                    <Text style={[styles.emptyTitle, { color: theme.text }]}>
                        Aún no has creado materias ni eventos.
                    </Text>
